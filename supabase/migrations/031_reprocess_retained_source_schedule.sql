-- Allow a newer deterministic parser to attach schedule candidates to an
-- immutable retained source snapshot without duplicating or changing evidence.
...
begin
  if pg_catalog.to_regprocedure('public.atlas_assert_service_role()') is null then
    raise exception 'Retained schedule reprocessing requires public.atlas_assert_service_role().';
  end if;
end;
$$;
...
comment on function public.atlas_reprocess_event_source_schedule(uuid, text, jsonb, text, text) is
  'Service-role-only deterministic reprocessing for exact immutable source snapshots; creates private schedule candidates and an append-only audit action only.';
