...
assert(atlasMapSource.includes("import { createPortal } from 'react-dom'") && fieldBlock.includes("typeof document !== 'undefined'") && fieldBlock.includes('createPortal(openClusterOverlay, document.body)'), 'result label cluster sheets should portal out of the transformed map layer');
assert(fieldBlock.includes('handleClusterEventSelect') && fieldBlock.includes('setOpenClusterId(null);') && fieldBlock.includes('onEventSelect(eventId);'), 'cluster sheet event selection should close the sheet before opening the event card');
assert(styleBlock.includes('resultTextClusterSheetMobile') && styleBlock.includes('position: \'fixed\'') && styleBlock.includes('maxHeight: \'min(48svh'), 'mobile result cluster sheet should use viewport-fixed sizing');
