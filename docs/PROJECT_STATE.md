...
The universal conversational Scout service is not connected. Submitting the composer now reveals one client-only generic response preview inside the existing translucent dock so the future answer layout can be reviewed; it performs no retrieval, source lookup, or event-specific reasoning, and the copy explicitly states that the universal service is not connected. The submitted question remains in the composer with focus preserved, and editing it clears the preview. The outer Scout surface remains one shared dock: the preview uses only spacing and a divider, while the logo, borderless transparent text-entry area, and submit control remain on one row with no nested text box. The fixed composer uses safe-area and visual-viewport keyboard insets, a 16px input to prevent iOS focus zoom, a minimum 44px send target, and responsive clearance for the taller preview state.
