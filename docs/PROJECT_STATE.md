...
- The August 9 Michigan artwork profile now uses Ray's 1024×1536 cloud-and-water map as the versioned desktop asset. The phone variant preserves the complete source at its top and extends only the lower ocean/cloud field to 1024×2216. Compact landscape and desktop retain the untouched source composition.
- Michigan presentation v3 opens portrait phones in a moderate reviewed pose: the Lower Peninsula is centered and readable, most of the U.P. remains visible, and reduced empty sky gives the southern Lower Peninsula more room above ASK without the artwork filling the frame. That reviewed pose is also portrait's minimum zoom, so pinch, reset, and Back restoration cannot fall into the undersized full-state composition. A bounded horizontal drag reveals the remaining U.P.; double tap restores the reviewed starting pose. At the zoom floor, vertical travel remains tightly bounded; as zoom increases, the upward pan boundary expands progressively so Detroit and the southern Lower Peninsula can be brought above ASK without permitting an unrestricted drift through the synthetic lower-water field. Active drag and pinch gestures disable movement easing synchronously, pan input travels at 1.7× finger movement, and touch-like WebKit pointers are accepted without ending on pointer-leave. Artwork, markers, clusters, and marker labels use identical transforms and settle timing, so dragging never creates marker drift. Saved homepage camera transforms now carry mobile-camera profile v2; stale transforms and transforms below the reviewed portrait floor are ignored while valid current Back-navigation state can still be restored.
- The only active illustrated-map calibration profile is now `michigan-clouds-artwork-calibration-v2`, scoped explicitly to the current clouds artwork and canonical event coordinates. It adds the current artwork's reviewed Lower Peninsula perspective shear and converts intrinsic artwork positions through the real rendered `cover`/`contain` geometry before placing markers, keeping Port Huron and Grand Haven on their illustrated shorelines even when mobile browser chrome shortens the viewport. The former painterly-map anchor interpolation, exact mobile U.P. targets, old regional overlay corrections, and their obsolete development workbenches remain removed. This remains reviewed illustrated-map presentation rather than geographic truth.
- Homepage ASK now has one structured, prose-free result contract shared by the map and event cards. Exact identity and complete structured intents retain the deterministic state resolver as the immediate fast path; open-ended queries use a bounded AI Gateway matcher over generalized nested Atlas knowledge, with a generalized all-fields local fallback when the provider is unavailable. The browser supplies only query and state identity; the server owns the complete publication-gated public candidate catalog, while the client rejects foreign returned IDs and continues to use its immediate deterministic result during resolution.
- ASK search knowledge now includes only the exact published Event Factory package ID and version already selected by public discovery, plus checked-in compatibility Event Hubs. Validated published manifests add source-bound schedules, participation, history, traditions, highlights, and practical facts without allowing drafts or private previews into the public corpus. Model matches select evidence fact IDs; compact factual match cues are then copied from those retained facts into desktop rows, mobile result cards, and the existing cluster Experience Deck without AI-authored explanation prose.
...
- Query-only broad search now uses accessible coded markers and real-coordinate, zoom-dependent numbered clusters instead of filling the statewide artwork with event-title tags. Cluster counts contain only the active result set. Exact identity still navigates directly, combined structured criteria intersect, and an empty result hides contradictory markers.
- Submitted homepage discovery queries now live in the public `q` URL parameter and are deterministically reconstructed from the reviewed Michigan catalog. Browser-history entries retain only presentation state: page scroll, live/upcoming rail position, profile-scoped map transform, open result cluster, Experience Deck visibility and active index, selected result, and exact-navigation suppression. Internal Event Hub transitions remain App Router client navigations, Back restores the homepage context without duplicated result storage, Forward returns to the same Event Hub, and exact-search returns do not immediately redirect again.
- The reusable Atlas Experience Deck remains the visually approved cluster rolodex. Tapping a production geographic cluster now opens that shared stack directly over the unchanged map, using the cluster's actual ordered event IDs and existing safe-media helpers. Recessed cards promote without navigation; the active card navigates to the existing `/events/[id]` route.
...
- General geographic clustering is implemented for the default map and ASK result sets. Membership uses canonical coordinates with stable spatial-hash grouping, cluster positions reuse the existing calibrated artwork positions, pinch and wheel zoom split groups, and close scale resolves to individual markers. The Experience Deck remains finished infrastructure; this milestone changes only the cluster source that opens it.
- An active ASK query remains visible in the existing ASK input with an inline clear control. Mobile event cards switch to the exact ordered ASK result set and return to live/upcoming events when cleared; desktop discovery cards consume the same result order. The only pending-state treatment is restrained compass motion.
- Live identity-bound outside research is not connected yet. Official URLs are retained as future research hints, but the current model may use only supplied Atlas knowledge and must not infer page contents. `docs/ASK_ATLAS_MAP_SEARCH_ARCHITECTURE.md` defines the research and large-catalog retrieval follow-up.
- ASK now emits one privacy-safe, schema-versioned operational outcome for each valid request. Structured hosting logs and a compact response header expose only matcher and broad volume, cue-coverage, and latency buckets; they omit query text and hashes, event identity, request identity, and persistent identifiers. No analytics table, cookie, or browser SDK was added.
- ASK also has a permanent seven-prompt synthetic benchmark bank covering waterside fireworks, tattoo art, parades, historical superlatives, participant income, family-friendly relative dates, and free jazz with public transit. Required builds run offline grounding, evidence-ID, ordering, and retained-cue checks without a model call; semantic cases remain explicitly marked for a future live-model evaluator with a frozen reference date.
...
- The illustrated map remains approximate. The v2 asset has its own versioned global calibration, but the reusable multi-state presentation-position resolver and reviewed regional/editorial override contract are not implemented yet.
