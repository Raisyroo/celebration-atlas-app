...
The approved Batch 0 result is a no-write crosswalk only: `MAC-001` remains linked to Armada Fair and `MAC-050` remains linked to Romeo Peach Festival; neither creates or updates a database row. The immutable Batch 1 manifest contains exact, non-executed payloads for `MAC-003`, `MAC-004`, `MAC-008`, `MAC-011`, `MAC-041`, `MAC-042`, and `MAC-049`; its SHA-256 is `d0203c6b9141f068a3a4c25ad6449ed641877117d7010fefabc535fb25bae9f2`. A fresh GET-only scan found no candidate-slug, candidate/source, operation-identity, exact county-identity, or seven-record preflight collision. All seven pass GET-only preflight but remain execution-ineligible because migration 018 is undeployed and apply approval has not been granted. The before-and-after deployed counts remained unchanged at 23 candidates, 40 candidate-source rows, 7 operation runs, and 19 canonical events; no Supabase write or staging RPC call occurred.

The seven-record Batch 1 manifest remains retained as historical preparation, but it is no longer the selected first-event pilot. The revised immutable, non-executed first-event pilot contains only `MAC-042` Bay-Rama Fishfly Festival, `MAC-049` Richmond Good Old Days Festival, and `MAC-026` Memphis Festival Days in `county-seed-first-event-pilot-staging-manifest-v2.json`; its SHA-256 is `d2d1c245c1c8ac4abea3a1fef1e21a9ab8da2adf7a05d0db6c8bfbaba3079fd8`. A fresh PostgREST GET-only preflight found no canonical event, candidate, source, slug, or idempotency collision for any of the three. Memphis retains its Phase B `Insufficient information` warning and missing address/organizer-source warnings rather than being silently promoted to a complete seed. Current-edition verification, geocoding or address resolution, evidence, Event Hub preparation, and Art remain future work for every pilot record. No image is included or selected; each Art gate reads `Waiting for Ray-provided image.`, and publication is blocked pending Ray's approved image. No migration, RPC, batch, candidate, discovery run, or other Supabase write was executed.

Migration `018_guard_county_seed_candidate_staging.sql` is proposed and review-only, is not deployed, and is not authorized for application in Phase C1. It contains fail-fast duplicate preconditions, narrow candidate-slug, candidate/source, and county-identity uniqueness guards, plus a service-role-only guarded wrapper around the existing intake RPC. County staging remains blocked until a separate human-approved schema task reapplies the read-only scan, reviews and deploys migration 018, verifies remote migration and generated-type parity, and approves an immutable execution manifest with its exact confirmation hash. No candidate staging, event research, publication, imagery, Supabase application-data write, or clustering has begun. The unrelated Event Hub validation failure expecting `Why Go / Schedule / Highlights / Plan` but receiving `Experience / Schedule / Highlights / Next Time` remains unresolved. See `docs/COUNTY_SEED_STAGING_READINESS.md`.
...
- The full `npm run build` does not currently pass: it reaches the unrelated pre-existing Event Hub column-name assertion expecting `Why Go / Schedule / Highlights / Plan` but receiving `Experience / Schedule / Highlights / Next Time`. Do not attribute this failure to the Experience Deck milestone or weaken the assertion without a separate reviewed task.
