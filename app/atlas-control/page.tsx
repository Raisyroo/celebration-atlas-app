...
export default async function AtlasControlPage() {
  ...
  const [runs, actions, reviewItems, completionRuns, eventPageVersions, sourceBundles, sourceSyntheses, factory, visualWorkflows] = supabase ? await Promise.all([
    ...
    supabase.from("atlas_review_items").select("id,operation_run_id,review_type,candidate_id,event_id,priority,status,recommended_action,evidence").eq("status", "open").order("priority", { ascending: false }).order("created_at", { ascending: false }).limit(12),
