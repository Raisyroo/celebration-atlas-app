...
export async function GET() {
  ...
  const { data, error } = await supabase.from("atlas_review_items").select("id,review_type,candidate_id,event_id,priority,status,recommended_action,evidence,created_at").eq("status", "open").order("priority", { ascending: false }).order("created_at", { ascending: false }).limit(12);
