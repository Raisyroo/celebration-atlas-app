...
import maplibregl, { type ErrorEvent, type Map, type MapGeoJSONFeature, type MapLayerMouseEvent, type GeoJSONSource } from 'maplibre-gl';
...
function describeMapLibreError(event?: MapLayerMouseEvent | ErrorEvent) {
  const mapErrorEvent = event as ErrorEvent | undefined;
...
function getFeatureEventId(feature: MapGeoJSONFeature | undefined) {
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      ...
      if (!maplibregl.supported({ failIfMajorPerformanceCaveat: true })) {
        const message = 'MapLibre reports that this browser/GPU does not support WebGL maps.';
        setMapError(message);
        setDiagnostic({ phase: 'unsupported', detail: message, level: 'error' });
        return;
      }

      setDiagnostic({ phase: 'constructor', detail: 'Creating MapLibre map from installed maplibre-gl package.' });
      try {
        ...
        map.on('click', CLUSTER_LAYER_ID, (event: MapLayerMouseEvent) => {
          ...
          const source = map.getSource(EVENT_SOURCE_ID) as GeoJSONSource;
    ...
    <div className="geospatialTestShell" style={styles.pageShell}>
      <section className="geospatialTestAudit" style={styles.auditPanel} aria-label="MapLibre geospatial audit summary">
        ...
        <ul className="geospatialTestAuditList" style={styles.auditList}>
          ...
          <li>Uses the installed maplibre-gl package directly; no CDN script or stylesheet injection is used.</li>
        </ul>
      </section>
