...
type ProjectionLabRegion = 'lower-peninsula' | 'straits-transition' | 'upper-peninsula';
...
const REGION_CLASSIFICATION_RULES = {
  lowerPeninsula: 'Default region, including all coordinates in the lower-peninsula polygon and northern Lower Peninsula locations south of the Straits band; Charlevoix (45.3181, -85.2584) stays here.',
  straitsTransition: 'Coordinates in the Mackinac/Straits seam band: latitude 45.55–46.05 and longitude -85.35–-83.75, unless a point falls clearly inside the Upper Peninsula polygon.',
  upperPeninsula: 'Coordinates inside the existing upper-peninsula polygon, plus a conservative latitude/longitude fallback for future western U.P. points west of -83.75 at/above 46.05°N.',
} as const;
...
const INITIAL_UPPER_PENINSULA_ANCHORS: UpperPeninsulaAnchor[] = [
  { id: 'escanaba', name: 'Escanaba', latitude: 45.7453, longitude: -87.0646, mapX: 31.8, mapY: 25.8, status: 'active' },
  { id: 'marquette', name: 'Marquette', latitude: 46.5436, longitude: -87.3954, mapX: 38.67, mapY: 17.32, status: 'active' },
  { id: 'sault-ste-marie', name: 'Sault Ste. Marie', latitude: 46.4953, longitude: -84.3453, mapX: 74.47, mapY: 20.55, status: 'active' },
  { id: 'houghton-future-slot', name: 'Houghton / future western U.P. slot', latitude: 47.1211, longitude: -88.5694, mapX: 21.6, mapY: 12.8, status: 'future-slot' },
];

const UP_LAB_REFERENCE_LOCATIONS = [
  { id: 'lab-escanaba', name: 'Escanaba lab reference', latitude: 45.7453, longitude: -87.0646 },
  { id: 'lab-marquette', name: 'Marquette lab reference', latitude: 46.5436, longitude: -87.3954 },
  { id: 'lab-sault-ste-marie', name: 'Sault Ste. Marie lab reference', latitude: 46.4953, longitude: -84.3453 },
  { id: 'lab-houghton', name: 'Houghton future western U.P. reference', latitude: 47.1211, longitude: -88.5694 },
  { id: 'lab-mackinac', name: 'Mackinac / Straits seam reference', latitude: 45.8492, longitude: -84.6189 },
  { id: 'lab-charlevoix', name: 'Charlevoix Lower Peninsula reference', latitude: 45.3181, longitude: -85.2584 },
];

const pointInPolygon = (latitude: number, longitude: number, polygonId: string) => {
  const polygon = MICHIGAN_GEO_POLYGONS.find((candidate) => candidate.id === polygonId);
  if (!polygon) return false;

  let inside = false;
  for (let index = 0, previousIndex = polygon.coordinates.length - 1; index < polygon.coordinates.length; previousIndex = index, index += 1) {
    const [xi, yi] = polygon.coordinates[index];
    const [xj, yj] = polygon.coordinates[previousIndex];
...
const classifyMichiganProjectionRegion = (latitude: number, longitude: number): ProjectionLabRegion => {
  if (pointInPolygon(latitude, longitude, 'upper-peninsula')) return 'upper-peninsula';

  const inStraitsBand = latitude >= STRAITS_TRANSITION_SETTINGS.minLatitude &&
  ...
  if (latitude >= STRAITS_TRANSITION_SETTINGS.maxLatitude && longitude <= STRAITS_TRANSITION_SETTINGS.maxLongitude) {
...
const interpolateUpperPeninsulaPosition = (
  latitude: number,
  longitude: number,
  anchors: UpperPeninsulaAnchor[],
) => {
  const weighted = anchors.reduce((accumulator, anchor) => {
    const latitudeDelta = latitude - anchor.latitude;
    const longitudeDelta = longitude - anchor.longitude;
    const distanceSquared = Math.max((latitudeDelta * latitudeDelta) + (longitudeDelta * longitudeDelta), 0.0001);
  ...
  return {
    x: Math.min(100, Math.max(0, weighted.xTotal / weighted.weightTotal)),
    y: Math.min(100, Math.max(0, weighted.yTotal / weighted.weightTotal)),
  };
};

const blendPositions = (baseline: { x: number; y: number }, candidate: { x: number; y: number }, weight: number) => ({
...
export default function MichiganArtworkCalibration() {
  ...
  const [upperPeninsulaAnchors, setUpperPeninsulaAnchors] = useState(INITIAL_UPPER_PENINSULA_ANCHORS);
  const [activeAnchorId, setActiveAnchorId] = useState(INITIAL_UPPER_PENINSULA_ANCHORS[0].id);
  ...
  const eventMarkers = useMemo(
    () => [
      ...sampleEvents.map((event) => ({ id: event.id, name: event.name, latitude: event.latitude, longitude: event.longitude })),
      ...UP_LAB_REFERENCE_LOCATIONS,
    ].map((event) => {
      const atlasPosition = latLngToAtlasPosition(event.latitude, event.longitude);
      const upperPeninsulaCandidatePosition = interpolateUpperPeninsulaPosition(
        event.latitude,
        event.longitude,
        upperPeninsulaAnchors,
      );
      const region = classifyMichiganProjectionRegion(event.latitude, event.longitude);
  ...
  const exportPayload = useMemo(() => ({
    ...
    separateRegionProjectionLab: {
      ...
      productionStatus: 'Developer-only preview; AtlasMap production marker placement and the restored Lower Peninsula baseline are not changed by this export.',
      lowerPeninsulaBaseline: {
        status: 'Preserved exactly through data/mapCalibration.ts latLngToAtlasPosition and MICHIGAN_MAP_ANCHORS.',
      ...
      interpolationMethod: {
        ...
        power: 2,
        source: 'real event latitude/longitude matched against named U.P. anchors only; no event IDs and no per-event x/y overrides',
      },
    },
  ...
  const moveActiveAnchor = (event: PointerEvent<HTMLDivElement>) => {
    ...
    setUpperPeninsulaAnchors((current) => current.map((anchor) => (
      anchor.id === activeAnchorId
        ? { ...anchor, mapX: formatNumber(Math.min(100, Math.max(0, x))), mapY: formatNumber(Math.min(100, Math.max(0, y))) }
        : anchor
    )));
  };
    ...
    <div style={styles.page}>
      ...
      <div style={styles.layout}>
        <section style={styles.stagePanel} aria-label={activeVariant.label}>
          <div style={{ ...styles.stage, aspectRatio: activeVariant.frameAspectRatio }} onPointerDown={moveActiveAnchor}>
            ...
            {eventMarkers.map(({ event, region, geoPosition, atlasPosition, productionPosition, upperPeninsulaCandidatePosition, seamBlendPosition }) => (
              <div key={event.id}>
                <span title={`${event.name} geographic reference (${region})`} style={{ ...styles.geoMarker, left: `${geoPosition.x}%`, top: `${geoPosition.y}%` }} />
                <span title={`${event.name} Lower Peninsula baseline`} style={{ ...styles.atlasMarker, left: `${atlasPosition.x}%`, top: `${atlasPosition.y}%` }} />
                <span title={`${event.name} calibrated production comparison`} style={{ ...styles.productionMarker, left: `${productionPosition.x}%`, top: `${productionPosition.y}%` }} />
                {region === 'upper-peninsula' || region === 'straits-transition' ? (
                  <span title={`${event.name} Upper Peninsula candidate projection`} style={{ ...styles.upCandidateMarker, left: `${upperPeninsulaCandidatePosition.x}%`, top: `${upperPeninsulaCandidatePosition.y}%` }} />
                ) : null}
                {region === 'straits-transition' ? (
                  <span title={`${event.name} blended Straits seam candidate`} style={{ ...styles.seamMarker, left: `${seamBlendPosition.x}%`, top: `${seamBlendPosition.y}%` }} />
                ) : null}
              </div>
            ))}
            {upperPeninsulaAnchors.map((anchor) => (
              <button
                key={anchor.id}
                type="button"
                onPointerDown={(event) => { event.stopPropagation(); setActiveAnchorId(anchor.id); }}
                title={`${anchor.name} U.P. artwork anchor`}
                style={{ ...styles.upAnchorMarker, ...(anchor.id === activeAnchorId ? styles.upAnchorMarkerActive : null), left: `${anchor.mapX}%`, top: `${anchor.mapY}%` }}
              >
                {anchor.name.slice(0, 1)}
              </button>
            ))}
          </div>
          <p style={styles.legend}><b>Blue dots</b> = raw latitude/longitude on geographic reference. <b>Gold rings</b> = Lower Peninsula baseline / production comparison from the restored anchor projection. <b>Green dots</b> = calibrated artwork comparison. <b>Magenta diamonds</b> = Upper Peninsula candidate projection. <b>Cyan squares</b> = Straits blended seam candidate. Lettered orange controls are draggable U.P. anchors.</p>
        </section>
        ...
        <aside style={styles.controls} aria-label="Calibration controls">
          ...
          <div style={styles.labPanel}>
            ...
            <p style={styles.correctionHelp}>Developer-only separate-region preview. Select an anchor, then click the artwork to revise its placement. Lower Peninsula baseline stays untouched; Charlevoix classifies as Lower Peninsula, Mackinac/Straits as transition, and Escanaba/Marquette/Sault/Houghton as U.P.</p>
            {upperPeninsulaAnchors.map((anchor) => (
              <button key={anchor.id} type="button" onClick={() => setActiveAnchorId(anchor.id)} style={{ ...styles.anchorButton, ...(anchor.id === activeAnchorId ? styles.anchorButtonActive : null) }}>
                {anchor.name}: {formatNumber(anchor.mapX)}%, {formatNumber(anchor.mapY)}% {anchor.status === 'future-slot' ? '(future slot)' : ''}
              </button>
            ))}
            <p style={styles.regionLabels}>Lower Peninsula baseline · Straits transition · Upper Peninsula candidate projection</p>
          </div>
...
const styles: Record<string, CSSProperties> = {
  ...
  upCandidateMarker: { position: 'absolute', width: 13, height: 13, background: '#ff4fd8', transform: 'translate(-50%, -50%) rotate(45deg)', boxShadow: '0 0 0 2px rgba(1,13,29,.75), 0 0 13px rgba(255,79,216,.95)' }, seamMarker: { position: 'absolute', width: 12, height: 12, background: '#61f3ff', transform: 'translate(-50%, -50%)', boxShadow: '0 0 0 2px rgba(1,13,29,.75), 0 0 13px rgba(97,243,255,.9)' }, upAnchorMarker: { position: 'absolute', width: 24, height: 24, borderRadius: '50%', border: '2px solid rgba(255,255,255,.9)', background: '#ff9d2e', color: '#140e08', fontWeight: 900, fontSize: 12, cursor: 'grab', transform: 'translate(-50%, -50%)', zIndex: 4 }, upAnchorMarkerActive: { background: '#fff1a8', boxShadow: '0 0 0 4px rgba(255,157,46,.35), 0 0 18px rgba(255,241,168,.9)' },
  controls: { flex: '1 1 320px', maxWidth: 360, minWidth: 0, border: '1px solid rgba(255,222,166,.22)', borderRadius: 22, padding: 18, background: 'rgba(8,12,18,.76)' }, panelTitle: { margin: '0 0 8px' }, notes: { color: 'rgba(248,239,217,.68)', lineHeight: 1.45 }, labPanel: { margin: '14px 0', padding: 12, borderRadius: 14, border: '1px solid rgba(255,79,216,.34)', background: 'rgba(255,79,216,.08)' }, labTitle: { margin: '0 0 8px', color: '#ffb8ef', fontSize: 15 }, anchorButton: { display: 'block', width: '100%', margin: '8px 0', padding: '8px 10px', borderRadius: 10, border: '1px solid rgba(255,222,166,.24)', background: 'rgba(255,255,255,.06)', color: '#ffe8b6', textAlign: 'left', cursor: 'pointer' }, anchorButtonActive: { background: '#ff9d2e', color: '#140e08', fontWeight: 900 }, regionLabels: { margin: '10px 0 0', color: '#ffe7b0', fontSize: 12, fontWeight: 800 }, correctionPanel: { margin: '14px 0', padding: 12, borderRadius: 14, border: '1px solid rgba(100,232,143,.24)', background: 'rgba(100,232,143,.08)' }, correctionTitle: { margin: '0 0 8px', color: '#aef0bd', fontSize: 15 }, correctionHelp: { margin: 0, color: 'rgba(220,255,230,.72)', fontSize: 13, lineHeight: 1.45 }, rangeLabel: { display: 'grid', gap: 8, margin: '16px 0', color: '#ffe7b0', fontSize: 14 }, rangeInput: { width: '100%' }, noRotation: { color: 'rgba(248,239,217,.66)', fontSize: 13, lineHeight: 1.45 }, copyButton: { width: '100%', padding: '11px 14px', borderRadius: 12, border: 0, background: '#f0bd55', color: '#140e08', fontWeight: 900, cursor: 'pointer' }, copyStatus: { color: '#9df0b0', fontWeight: 800 }, pre: { maxHeight: 320, overflow: 'auto', padding: 12, borderRadius: 12, background: 'rgba(0,0,0,.38)', color: '#dcecff', fontSize: 12, whiteSpace: 'pre-wrap' },
};
