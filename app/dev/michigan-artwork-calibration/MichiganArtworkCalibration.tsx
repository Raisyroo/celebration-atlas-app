...
const regionalCorrectionSummary = `U.P./Straits regional layer: mobile-only; Lower Peninsula remains 0 weight south of ${formatCorrectionLatitude(MICHIGAN_UPPER_PENINSULA_STRAITS_CORRECTION.transitionStartLatitude)}, blends through the Straits to ${formatCorrectionLatitude(MICHIGAN_UPPER_PENINSULA_STRAITS_CORRECTION.upperPeninsulaStartLatitude)}, then applies up to ${formatCorrectionOffset(MICHIGAN_UPPER_PENINSULA_STRAITS_CORRECTION.maxXOffsetPercent)} x and ${formatCorrectionOffset(MICHIGAN_UPPER_PENINSULA_STRAITS_CORRECTION.maxYOffsetPercent)} y with ${formatNumber(MICHIGAN_UPPER_PENINSULA_STRAITS_CORRECTION.xScaleFromStraitsCenter)} local x scale.`;
...
export default function MichiganArtworkCalibration() {
  ...
  const exportPayload = useMemo(() => ({
    ...
    geographicRegions: {
      lowerPeninsula: 'Regional U.P./Straits correction weight is 0 south of transitionStartLatitude, preserving Lower Michigan calibration.',
      transitionBand: 'Smoothstep blend from transitionStartLatitude to upperPeninsulaStartLatitude, with Mackinac / Straits longitude gating.',
      upperPeninsula: 'Full correction north of upperPeninsulaStartLatitude, plus west-longitude U.P. handling for Escanaba / Marquette style targets.',
    },
    upperPeninsulaStraitsCorrection: {
      ...MICHIGAN_UPPER_PENINSULA_STRAITS_CORRECTION,
      interpolation: 'geography-based smoothstep weights; no event IDs or manual marker overrides',
    },
  ...
  return (
    <div style={styles.page}>
      <header style={styles.header}>
        ...
        <p style={styles.correctionNote}>{mobileCorrectionSummary} {regionalCorrectionSummary} Real event latitude/longitude remains unchanged, and there are no per-event marker overrides.</p>
      </header>
        ...
        <aside style={styles.controls} aria-label="Calibration controls">
          ...
          {activeVariant.id === 'mobile' ? (
            <div style={styles.correctionPanel}>
              ...
              <h3 style={styles.correctionTitle}>U.P. / Straits regional correction</h3>
              <p style={styles.correctionHelp}>{regionalCorrectionSummary} The transition band is latitude-led and longitude-gated around Mackinac so this remains a regional model, not per-marker editing.</p>
            </div>
          ) : null}
          ...
          <section style={styles.validationPanel} aria-label="Geographic validation targets">
            <h3 style={styles.correctionTitle}>Validation target regional weights</h3>
            {validationTargets.map((target) => {
              const debug = getMichiganRegionalCorrectionDebug(target.latitude, target.longitude);
              return (
                <p key={target.name} style={styles.validationRow}>
                  <strong>{target.name}</strong>: {debug.zone}, weight {formatNumber(debug.regionalWeight)}, x {formatCorrectionOffset(debug.xOffsetPercent + debug.xScaleOffsetPercent)}, y {formatCorrectionOffset(debug.yOffsetPercent)}
                </p>
              );
            })}
          </section>
...
const styles: Record<string, CSSProperties> = {
  ...
  controls: { flex: '1 1 320px', maxWidth: 360, minWidth: 0, border: '1px solid rgba(255,222,166,.22)', borderRadius: 22, padding: 18, background: 'rgba(8,12,18,.76)' }, panelTitle: { margin: '0 0 8px' }, notes: { color: 'rgba(248,239,217,.68)', lineHeight: 1.45 }, correctionPanel: { margin: '14px 0', padding: 12, borderRadius: 14, border: '1px solid rgba(100,232,143,.24)', background: 'rgba(100,232,143,.08)' }, correctionTitle: { margin: '0 0 8px', color: '#aef0bd', fontSize: 15 }, correctionHelp: { margin: 0, color: 'rgba(220,255,230,.72)', fontSize: 13, lineHeight: 1.45 }, rangeLabel: { display: 'grid', gap: 8, margin: '16px 0', color: '#ffe7b0', fontSize: 14 }, rangeInput: { width: '100%' }, noRotation: { color: 'rgba(248,239,217,.66)', fontSize: 13, lineHeight: 1.45 }, copyButton: { width: '100%', padding: '11px 14px', borderRadius: 12, border: 0, background: '#f0bd55', color: '#140e08', fontWeight: 900, cursor: 'pointer' }, copyStatus: { color: '#9df0b0', fontWeight: 800 }, validationPanel: { margin: '14px 0', padding: 12, borderRadius: 14, border: '1px solid rgba(98,199,255,.24)', background: 'rgba(98,199,255,.07)' }, validationRow: { margin: '6px 0', color: 'rgba(220,238,255,.76)', fontSize: 12, lineHeight: 1.35 }, pre: { maxHeight: 320, overflow: 'auto', padding: 12, borderRadius: 12, background: 'rgba(0,0,0,.38)', color: '#dcecff', fontSize: 12, whiteSpace: 'pre-wrap' },
};
