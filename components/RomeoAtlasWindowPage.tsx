...
const styles: Record<string, CSSProperties> = {
  ...
  atlasWindow: { alignSelf: 'stretch', position: 'relative', zIndex: 2, minHeight: 0, marginTop: 'clamp(2.45rem, 7svh, 4rem)', borderRadius: '1.55rem', overflow: 'hidden', border: 'none', outline: 'none', background: 'rgba(6,10,16,.42)', boxShadow: '0 24px 80px rgba(0,0,0,.55)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)' },
  festivalMemory: { position: 'absolute', inset: '-2% -4%', zIndex: 0, opacity: 0.38, backgroundSize: 'cover', backgroundPosition: '50% 46%', backgroundRepeat: 'no-repeat', filter: 'brightness(0.72) saturate(1.03) contrast(1.08) blur(0.1px)', mixBlendMode: 'screen', pointerEvents: 'none', transform: 'translateZ(0)' },
  memorySmoke: { position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none', background: 'radial-gradient(circle at 46% 36%, rgba(255,190,112,0.1), rgba(9,12,20,0.34) 54%, rgba(2,4,9,0.58) 100%), linear-gradient(180deg, rgba(3,6,13,0.28), rgba(8,10,16,0.46)), radial-gradient(circle at 18% 82%, rgba(0,0,0,0.36), transparent 38%)', boxShadow: 'inset 0 0 64px rgba(0,0,0,0.5), inset 0 0 120px rgba(1,3,8,0.42)' },
