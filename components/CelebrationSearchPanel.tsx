...
export default function CelebrationSearchPanel({
  ...
  const showMichiganCta =
    result?.command.scope === "state" &&
    result.command.stateSlug === "michigan";
    ...
    <section aria-label="Celebration Search command panel" style={styles.panel}>
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <div style={styles.inputShell}>
          <input
            ...
            placeholder="Ask Celebration Atlas"
            style={styles.input}
            type="search"
          />
      ...
      {result ? (
        <div style={styles.resultPanel} aria-live="polite">
          ...
          {result.command.needsClarification &&
          result.command.clarificationQuestion ? (
            <p style={styles.clarification}>
              {result.command.clarificationQuestion}
            </p>
          ) : null}
          ...
          {eventMatch ? (
            <div style={styles.michiganCtaGroup}>
              <Link
                href={`/events/${eventMatch.slug}`}
                style={styles.michiganCta}
                aria-label={`Open ${eventMatch.name}`}
              >
                <span style={styles.michiganCtaKicker}>Event doorway</span>
                <span style={styles.michiganCtaText}>
                  Open {eventMatch.name}
                </span>
              </Link>
              <p style={styles.michiganCtaNote}>
                Known event match. The national preview highlights the Michigan
                doorway and offers the existing event route.
              </p>
            </div>
          ) : null}
          ...
          {showMichiganCta ? (
            <div style={styles.michiganCtaGroup}>
              <Link
                href="/"
                style={styles.michiganCta}
                aria-label="Open Michigan Atlas"
              >
                <span style={styles.michiganCtaKicker}>State doorway</span>
                <span style={styles.michiganCtaText}>Open Michigan Atlas</span>
              </Link>
              <p style={styles.michiganCtaNote}>
                Manual doorway only. No query parameters, command state, or
                automatic navigation are sent to the Michigan Atlas.
              </p>
            </div>
          ) : null}
        </div>
