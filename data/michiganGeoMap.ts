...
export const MICHIGAN_REFERENCE_SVG = {
  ...
  sourceUrl:
    "https://commons.wikimedia.org/wiki/File:USA_Michigan_location_map.svg",
...
export const isPointInMichiganGeoPolygon = (
  latitude: number,
  longitude: number,
  polygonId: string,
) => {
  const polygon = MICHIGAN_GEO_POLYGONS.find(
    (candidate) => candidate.id === polygonId,
  );
  if (!polygon) return false;

  let inside = false;
  for (
    let index = 0, previousIndex = polygon.coordinates.length - 1;
    index < polygon.coordinates.length;
...
const MICHIGAN_STRAITS_TRANSITION_EXCLUSION = {
  minLatitude: 45.55,
  maxLatitude: 46.05,
  minLongitude: -85.35,
  maxLongitude: -83.75,
...
const isPointInMichiganStraitsTransitionExclusion = (
  latitude: number,
  longitude: number,
) =>
  latitude >= MICHIGAN_STRAITS_TRANSITION_EXCLUSION.minLatitude &&
  latitude <= MICHIGAN_STRAITS_TRANSITION_EXCLUSION.maxLatitude &&
  longitude >= MICHIGAN_STRAITS_TRANSITION_EXCLUSION.minLongitude &&
  longitude <= MICHIGAN_STRAITS_TRANSITION_EXCLUSION.maxLongitude;

export const isPointInMichiganUpperPeninsula = (
  latitude: number,
  longitude: number,
) =>
  isPointInMichiganGeoPolygon(latitude, longitude, "upper-peninsula") &&
  !isPointInMichiganStraitsTransitionExclusion(latitude, longitude);
...
const getMichiganArtworkRegionalCorrection = (
  latitude: number,
  longitude: number,
) =>
...
export function latLngToMichiganMapPosition(
  latitude: number,
  longitude: number,
): MichiganMapPosition {
...
export const MICHIGAN_SVG_REFERENCE_POINTS: readonly MichiganSvgReferencePoint[] =
  [
    ...
    const svgPosition = latLngToMichiganSvgPosition(
      point.latitude,
      point.longitude,
    );
...
export function michiganGeoPolygonToSvgPath(
  coordinates: readonly MichiganGeoPoint[],
) {
  return (
    coordinates
